...
@interface CoreDataPhotoViewController () <UIPopoverControllerDelegate>
@property (nonatomic, strong) Photo *cdPhoto;
@end

@implementation CoreDataPhotoViewController
...
-(void)showAvailableVacationsPopup:(NSArray *)vacations animated:(BOOL)animated {
    ...
    if(vacations) {
        if(self.popover) {
            ...
            if(!self.popoverData) {
                self.popoverData = [[VacationFilesPopupTableViewController alloc] initWithStyle:UITableViewStylePlain];
    ...
    if(self.isOnVacation) {
        ...
        [VacationHelper openVacation:self.vacationName withExtension:nil usingBlock:^(UIManagedDocument *vacation) {
            if([self.visitButton.title isEqualToString:VISIT_LABEL]) {
                self.cdPhoto = [Photo photoWithDictionaryInfo:self.photo inManagedObjectContext:vacation.managedObjectContext];
                isVisited = YES;
            }
            else {
                // remove photo from vacation
                if(self.cdPhoto) {
                    [self.cdPhoto.managedObjectContext deleteObject:self.cdPhoto];
                    self.cdPhoto = nil;
                    isVisited = NO;
                }
            }
...
-(void)setupVisitButton {
    ...
    if(self.vacationName) {
        [VacationHelper openVacation:self.vacationName withExtension:nil usingBlock:^(UIManagedDocument *vacation) {
            // we're now in the main thread
            if((self.cdPhoto = [Photo photoWithDictionaryInfo:self.photo existsInManagedObjectContext:vacation.managedObjectContext])) {
                self.visitButton.title = UNVISIT_LABEL;
            } else {
